		...
		try {
			if (Objects.equals(name, "AmazonCorrettoCryptoProvider")) {
				...
				if (!GetterUtil.getBoolean(
						isExperimentalFipsMethod.invoke(fipsProvider)) ||
					GetterUtil.getBoolean(isFipsMethod.invoke(fipsProvider))) {
					return;
				}

				throw new SecurityException(
					"AmazonCorrettoCryptoProvider must be a non experimental " +
						"FIPS build");
			}
