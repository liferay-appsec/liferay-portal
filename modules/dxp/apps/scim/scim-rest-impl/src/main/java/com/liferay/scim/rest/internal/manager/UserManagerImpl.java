...
public class UserManagerImpl implements UserManager {
	...
	public UsersGetResponse listUsersWithGET(
		...
		Predicate wherePredicate = _buildWherePredicate(
			...
			UserTable.INSTANCE.status.eq(WorkflowConstants.STATUS_APPROVED)
		);
		...
		return new UsersGetResponse(
			_countAll(
				UserTable.INSTANCE, UserTable.INSTANCE.userId,
				_userLocalService, wherePredicate),
			TransformUtil.transform(
				users,
				user -> {
	...
	private <T extends BaseTable<T>, U> List<U> _fetch(
		BaseTable<T> baseTable, Expression<Long> expression,
		PersistedModelLocalService persistedModelLocalService,
		Predicate wherePredicate, int start, Integer end) {
		...
		if (end != null) {
			GroupByStep groupByStep = (GroupByStep)dslQuery;

			dslQuery = groupByStep.limit(start, start + end);
		}
