...
public class ResourceTypesResourceImpl extends BaseResourceTypesResourceImpl {
	...
	private String _getResourceType(String id) throws AbstractCharonException {
		if (_resourceTypesMap.containsKey(id)) {
			JSONObject resourceTypeJSONObject = _read(
				_resourceTypesMap.get(id));
	...
	private String _getResourceTypes() throws AbstractCharonException {
		return JSONUtil.put(
			"itemsPerPage", 2
		).put(
			"Resources",
			...
			"totalResults", _resourceTypesMap.size()
		).toString();
	}
		...
		try {
			...
			if (Validator.isNull(id)) {
				return new SCIMResponse(
					ResponseCodeConstants.CODE_OK, _getResourceTypes(),
					_getResponseHeaders());
			}

			String resourceType = _getResourceType(id);
			...
			return new SCIMResponse(
				ResponseCodeConstants.CODE_OK, resourceType,
				_getResponseHeaders());
		}
	...
	private JSONObject _read(String fileName) throws InternalErrorException {
		try {
			...
			throw new InternalErrorException(
				"Error reading resource type file " + fileName);
		}
	}
	...
	private final Map<String, String> _resourceTypesMap = Map.of(
