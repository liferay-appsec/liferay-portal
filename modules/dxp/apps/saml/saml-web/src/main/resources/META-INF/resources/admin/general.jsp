...
boolean roleSp = StringUtil.equalsIgnoreCase(samlProviderConfiguration.role(), SamlProviderConfigurationKeys.SAML_ROLE_SP) || StringUtil.equalsIgnoreCase(samlProviderConfiguration.role(), SamlProviderConfigurationKeys.SAML_MULTIROLE);
	...
	<aui:fieldset>
		...
		<aui:select disabled="<%= samlRoleIdpOptionDisabled %>" helpMessage="<%= samlRoleHelpMessage %>" label="saml-role" name='<%= "settings--" + PortletPropsKeys.SAML_ROLE + "--" %>' required="<%= !samlRoleIdpOptionDisabled %>">
			<c:if test='<%= FeatureFlagManagerUtil.isEnabled("LPD-29737") %>'>
				<aui:option label="identity-and-service-provider" selected="<%= samlRole.equals(SamlProviderConfigurationKeys.SAML_MULTIROLE) %>" value="<%= SamlProviderConfigurationKeys.SAML_MULTIROLE %>" />
			</c:if>
