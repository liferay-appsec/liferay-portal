...
String requestParamsJSON = (String)request.getAttribute("requestParamsTemplate");
...
if ((authServerIssuer.length() > 0) && (clientId.length() > 0)) {
	OAuthClientEntry oAuthClientEntry = OAuthClientEntryServiceUtil.fetchOAuthClientEntry(companyId, authServerIssuer, clientId);

	oAuthClientEntryId = oAuthClientEntry.getOAuthClientEntryId();

	infoJSON = oAuthClientEntry.getInfoJSON();

	requestParamsJSON = oAuthClientEntry.getRequestParamsJSON();
}
		...
		<div class="sheet">
			<clay:row>
				...
				>
					...
					<aui:input helpMessage="oauth-client-request-params-json-help" label="OAuth Client Request Parameters" name="requestParamsJSON" style="min-height: 200px;" type="textarea" />
...
<aui:script>
	...
	function <portlet:namespace />doSubmit() {
		...
		document.getElementById('<portlet:namespace />requestParamsJSON').value = JSON.stringify(JSON.parse(document.getElementById('<portlet:namespace />requestParamsJSON').value), null, 0);
	...
	function <portlet:namespace />init() {
		...
		document.getElementById('<portlet:namespace />requestParamsJSON').value = JSON.stringify(JSON.parse('<%= requestParamsJSON %>'), null, 4);
	}
