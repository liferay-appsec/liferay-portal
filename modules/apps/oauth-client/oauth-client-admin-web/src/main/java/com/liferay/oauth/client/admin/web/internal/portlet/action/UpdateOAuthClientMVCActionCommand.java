...
public class UpdateOAuthClientMVCActionCommand implements MVCActionCommand {
	...
	public boolean processAction(
		...
		String authServerIssuer = ParamUtil.getString(
			actionRequest, "authServerIssuer");

		String infoJSON = ParamUtil.getString(actionRequest, "infoJSON");

		String requestParamsJSON = ParamUtil.getString(
			actionRequest, "requestParamsJSON");

		try {
			if (oAuthClientEntryId > 0) {
				_oAuthClientEntryService.updateOAuthClientEntry(
					oAuthClientEntryId, authServerIssuer, infoJSON,
					requestParamsJSON);
			}
			else {
				_oAuthClientEntryService.addOAuthClientEntry(
					themeDisplay.getCompanyId(), themeDisplay.getUserId(),
					authServerIssuer, infoJSON, requestParamsJSON);
			}
		}
