...
create unique index IX_328DCB29 on OAuthClientASLocalMetadata (companyId, issuer[$COLUMN_LENGTH:255$]);
create index IX_CEF2762B on OAuthClientASLocalMetadata (companyId, localWellKnownEnabled);
create unique index IX_E5878996 on OAuthClientASLocalMetadata (companyId, localWellKnownURI[$COLUMN_LENGTH:255$]);
create unique index IX_B2201FE9 on OAuthClientASLocalMetadata (companyId, oAuthASLocalWellKnownURI[$COLUMN_LENGTH:255$]);
...
create unique index IX_FEC415C2 on OAuthClientEntry (companyId, authServerWellKnownURI[$COLUMN_LENGTH:255$], clientId[$COLUMN_LENGTH:255$]);
...
create unique index IX_E3839C6 on OAuthClientPRLocalMetadata (companyId, localWellKnownURI[$COLUMN_LENGTH:255$]);
create unique index IX_B54739F0 on OAuthClientPRLocalMetadata (companyId, protectedResourceURI[$COLUMN_LENGTH:255$]);
