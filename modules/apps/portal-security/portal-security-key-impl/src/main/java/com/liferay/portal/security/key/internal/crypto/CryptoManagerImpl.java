...
public class CryptoManagerImpl implements CryptoManager {
	...
	public CryptoServiceResult<byte[]> wrap(
		...
		if (!Objects.equals(
				keyToWrapReference.getProviderId(),
				masterKeyReference.getProviderId())) {

			throw new CryptoManagerException(
				StringBundler.concat(
					"Key to wrap provider ", keyToWrapReference.getProviderId(),
		...
		if (_log.isInfoEnabled()) {
			_log.info(
				StringBundler.concat(
					...
					serviceIndicator.getSecurityFunction(), " (approved: ",
	...
	private CryptoServiceResult<KeyReference> _generate(
		...
		CryptoServiceResult<String> cryptoServiceResult = keyGenerator.generate(
			_getCryptoVaultProvider(companyId, resolvedProviderId), algorithm,
	...
	private CryptoVaultProvider _getCryptoVaultProvider(
		...
		if (serviceTrackerMap == null) {
			throw new CryptoManagerException(
				"Service tracker map is not initialized");
		}

		List<CryptoVaultProvider> cryptoVaultProviders =
			serviceTrackerMap.getService(resolvedProviderId);
		...
		if (Objects.equals(providerId, StringPool.STAR)) {
			...
			if (companyId == 0L) {
				if (providerRole == ProviderRole.KEK) {
					resolvedProviderId = activeProfile.getSystemKEKProviderId();
				}
				else {
					resolvedProviderId = activeProfile.getSystemDEKProviderId();
				}
			}
			else if (providerRole == ProviderRole.KEK) {
				resolvedProviderId = activeProfile.getCompanyKEKProviderId();
			}
			else {
				resolvedProviderId = activeProfile.getCompanyDEKProviderId();
			}

			if (resolvedProviderId == null) {
				throw new CryptoManagerException(
					StringBundler.concat(
						"Active KeyManagerProfile resolved a null provider ID ",
						"for role: ", providerRole, " and company ID: ",
						companyId));
			}
	...
	private volatile ServiceTrackerMap<String, List<CryptoVaultProvider>>
		_serviceTrackerMap;
