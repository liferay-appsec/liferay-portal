...
public class WrappedCompanyKey {
	...
	public static WrappedCompanyKey parse(
		...
		String encodedCiphertext = body.substring(pipeIndex + 1);

		if (!_isBase64(encodedCiphertext)) {
			throw new CompanyKeyResolutionException(
	...
	public String serialize() {
		return StringBundler.concat(
			...
			Base64.encode(_ciphertext), StringPool.CLOSE_CURLY_BRACE);
	}

	private static boolean _isBase64(String string) {
		int length = string.length();

		int paddingIndex = length;

		while ((paddingIndex > 0) &&
			   (string.charAt(paddingIndex - 1) == CharPool.EQUAL)) {

			paddingIndex--;
		}

		if ((paddingIndex == 0) || ((length - paddingIndex) > 2)) {
			return false;
		}
		...
		for (int i = 0; i < paddingIndex; i++) {
			...
			if (((c < CharPool.LOWER_CASE_A) || (c > CharPool.LOWER_CASE_Z)) &&
				((c < CharPool.UPPER_CASE_A) || (c > CharPool.UPPER_CASE_Z)) &&
