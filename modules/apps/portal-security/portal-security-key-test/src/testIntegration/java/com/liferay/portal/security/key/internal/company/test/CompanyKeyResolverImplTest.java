...
public class CompanyKeyResolverImplTest {
	...
	public void setUp() throws Exception {
		...
		ConfigurationTestUtil.saveConfiguration(
			...
			HashMapDictionaryBuilder.<String, Object>put(
				"companyKEKProviderId", TestCompanyCryptoProvider.PROVIDER_ID
			).build());
	...
	public void testDeserializeKey() throws Exception {
		...
		Key legacyKey = EncryptorUtil.generateKey();

		deserializedKey = CompanyKeyUtil.deserializeKey(
			companyId, EncryptorUtil.serializeKey(legacyKey));

		Assert.assertEquals(
			decryptCount + 1, _testCompanyCryptoProvider.getDecryptCount());
		Assert.assertEquals(legacyKey, deserializedKey);
	}
