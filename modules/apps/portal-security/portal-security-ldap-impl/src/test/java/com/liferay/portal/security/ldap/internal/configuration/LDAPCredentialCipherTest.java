...
public class LDAPCredentialCipherTest {
	...
	public void setUp() throws Exception {
		_ldapCredentialCipher = new LDAPCredentialCipherImpl();
		...
		ReflectionTestUtil.setFieldValue(
			_ldapCredentialCipher, "_portal", _portal);
	...
	public void testResolveDecryptsEncryptedValue() {
		try (MockedStatic<EncryptorUtil> encryptorUtilMockedStatic =
			...
			String resolvedString = _ldapCredentialCipher.resolve(
				_COMPANY_ID, "{ENC}cipher");

			Assert.assertEquals("plaintext", resolvedString);
		}
	}
	...
	public void testResolveReturnsOriginalOnDecryptFailure() {
		try (MockedStatic<EncryptorUtil> encryptorUtilMockedStatic =
			...
			Assert.assertEquals("{ENC}cipher", resolvedString);
		}
	}
	...
	@Test
	public void testResolveUsesDefaultCompanyWhenCompanyIdIsZero() {
		try (MockedStatic<EncryptorUtil> encryptorUtilMockedStatic =
				Mockito.mockStatic(EncryptorUtil.class)) {
			...
			Assert.assertEquals(
				"plaintext", _ldapCredentialCipher.resolve(0L, "{ENC}cipher"));
		}
	}
