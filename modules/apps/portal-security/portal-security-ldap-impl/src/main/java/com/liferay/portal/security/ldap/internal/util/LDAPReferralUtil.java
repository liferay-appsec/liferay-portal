...
public class LDAPReferralUtil {
		...
		try {
			while (!dirContexts.isEmpty()) {
				...
				try {
					...
					referralCount = _followSafeReferrals(
	...
	private static void _close(DirContext dirContext) {
		try {
			dirContext.close();
		}
			...
			if (_log.isDebugEnabled()) {
				_log.debug(
					"Unable to close directory context", namingException);
			}
		}
	}

	private static int _followSafeReferrals(
				...
				if (referralCount < _MAX_REFERRAL_COUNT) {
					...
					_log.debug(
						"Stopped following LDAP referrals after reaching the " +
							"maximum count of " + _MAX_REFERRAL_COUNT);
				}
			}
			else if (_log.isDebugEnabled()) {
				_log.debug(
