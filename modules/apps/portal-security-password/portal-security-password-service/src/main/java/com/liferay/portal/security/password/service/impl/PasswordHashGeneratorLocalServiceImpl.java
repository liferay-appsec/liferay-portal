...
public class PasswordHashGeneratorLocalServiceImpl
	 ...
	 * @param  metaJSONString the password hash generator meta in json format
	 * @return PasswordHashGenerator
	 */
	private PasswordHashGenerator _addPasswordHashGenerator(
			String name, String metaJSONString)
		throws Exception {
		...
		if (Validator.isNull(metaJSONString)) {
			metaJSONObject = new JSONObject();
		}
		else {
			metaJSONObject = new JSONObject(metaJSONString);
		}
		...
		if (lastPasswordHashGenerator == null) {
			return _addPasswordHashGeneratorCommon(name, metaJSONObject);
		}
		...
		JSONObject storedHashGeneratorMetaJSONObject = new JSONObject(
			lastPasswordHashGenerator.getHashGeneratorMeta());

		if (!metaJSONObject.similar(storedHashGeneratorMetaJSONObject)) {
			_deleteLastHashGeneratorIfNotUsed();
	 ...
	 *
	 * @param  name the password hash generator name
	 * @param  metaJSONObject the password hash generator meta
	 * @return PasswordHashGenerator
	 */
	private PasswordHashGenerator _addPasswordHashGeneratorCommon(
		String name, JSONObject metaJSONObject) {
		...
		passwordHashGenerator.setHashGeneratorMeta(metaJSONObject.toString());
	...
	private static final String _PEPPER_APP_ID =
