...
public class OpenIdConnectProviderManagedServiceFactory
	...
	private String _generateInfoJSON(Dictionary<String, ?> properties) {
		return JSONUtil.put(
			"client_id", properties.get("openIdConnectClientId")
		).put(
			"client_name", _generateClientName(properties)
		).put(
			"client_secret", properties.get("openIdConnectClientSecret")
		).put(
			"grant_types",
	...
	private String _generateMetadataJSON(Dictionary<String, ?> properties) {
		return JSONUtil.put(
			"authorization_endpoint", properties.get("authorizationEndPoint")
		).put(
			"id_token_signing_alg_values_supported",
			_generateMetadataJSONArray(properties, "idTokenSigningAlgValues")
		).put(
			"issuer", properties.get("issuerURL")
		).put(
			"jwks_uri", properties.get("jwksURI")
		).put(
			"scopes_supported", _generateMetadataJSONArray(properties, "scopes")
		).put(
			"subject_types_supported",
			_generateMetadataJSONArray(properties, "jwksURI")
		).put(
			"token_endpoint", properties.get("tokenEndPoint")
		).put(
			"userinfo_endpoint", properties.get("userInfoEndPoint")
		).toString();
	}

	private JSONArray _generateMetadataJSONArray(
		...
		if (propertyName.equals("scopes")) {
			String scopes = GetterUtil.getString(properties.get("scopes"));

			values = scopes.split(" ");
		}
		else {
			values = GetterUtil.getStringValues(properties.get(propertyName));
		}
	...
	private void _updateCompanyIdProviderNameOAuthClientEntryIds(
		...
		oAuthClientEntryIds.put(providerName, oAuthClientEntryId);
	}
