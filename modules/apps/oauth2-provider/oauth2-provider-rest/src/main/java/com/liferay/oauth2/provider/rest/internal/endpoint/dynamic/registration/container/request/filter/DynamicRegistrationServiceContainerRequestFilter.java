...
public class DynamicRegistrationServiceContainerRequestFilter
	...
	private void _validateOpenRegistrationRateLimit(
		...
		long windowStart =
			(time / _RATE_LIMIT_WINDOW_MILLIS) * _RATE_LIMIT_WINDOW_MILLIS;
		...
		long retryAfterSeconds =
			((windowStart + _RATE_LIMIT_WINDOW_MILLIS) - time) / 1000;
		...
		if (_log.isWarnEnabled()) {
			_log.warn(
				StringBundler.concat(
					"Open registration rate limit exceeded for company ",
					companyId, " from \"", clientHost, "\"; retry after ",
