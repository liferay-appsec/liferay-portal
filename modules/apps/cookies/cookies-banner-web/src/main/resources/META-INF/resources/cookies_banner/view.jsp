...
<c:if test="<%= (boolean)request.getAttribute(CookiesBannerWebKeys.FLOATING_ICON_ENABLED) %>">
	...
	<%
	String icon = (String)request.getAttribute(CookiesBannerWebKeys.FLOATING_ICON);
	long imageId = (long)request.getAttribute(CookiesBannerWebKeys.CUSTOM_FLOATING_ICON_IMAGE_ID);
	%>

	<c:choose>
		<c:when test='<%= Objects.equals("custom", icon) %>'>
			<img class="floating-icon-image align-items-center d-none  justify-content-center ml-3" id="<portlet:namespace />floatingIconButton" name="<portlet:namespace />floatingIconButton" src="<%= (imageId == 0) ? themeDisplay.getPathThemeImages() + "/spacer.png" : themeDisplay.getPathImage() + "/floating_icon?img_id=" + imageId %>" />
		</c:when>
		<c:otherwise>
			<clay:button
