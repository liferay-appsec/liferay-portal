	...
	>
		<clay:row>
			...
			>
				...
				<clay:content-col>
					<button aria-label="<%= LanguageUtil.get(request, "cookie-configuration") %>" class="btn btn-link btn-sm" id="<portlet:namespace />configurationButton" type="button">
				...
				<c:if test="<%= cookiesBannerDisplayContext.isIncludeDeclineAllButton() %>">
					<clay:content-col>
						<button aria-label="<%= LanguageUtil.get(request, "decline-all-cookies") %>" class="btn btn-secondary btn-sm" id="<portlet:namespace />declineAllButton" type="button">
							<liferay-ui:message key="decline-all" />
						</button>
					</clay:content-col>
				</c:if>
