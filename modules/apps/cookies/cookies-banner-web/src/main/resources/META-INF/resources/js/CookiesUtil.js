...
function deleteStoredCookie(name) {
	fetch(`/o/cookies/v1.0/consent-preference/by-name/${name}`, {
		headers: HEADERS,
		method: 'DELETE',
	});
}
...
async function fetchStoredCookie(name) {
	return await fetch(`/o/cookies/v1.0/consent-preference/by-name/${name}`, {
		headers: HEADERS,
		method: 'GET',
	})
		.then((response) => {
...
export function setCookie(consentRenewalPeriod, name, value) {
	...
	if (
		...
		fetch('/o/cookies/v1.0/consent-preferences', {
