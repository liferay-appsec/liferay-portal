...
public class HttpImplTest {
	...
	@Test
	public void testGetRequestConfigBuilderWithSocketTimeout()
		throws Exception {

		RequestConfig.Builder requestConfigBuilder = ReflectionTestUtil.invoke(
			_httpImpl, "_getRequestConfigBuilder",
			new Class<?>[] {int.class, URI.class, int.class}, 10000,
			URI.create("http://" + RandomTestUtil.randomString()), 5000);

		RequestConfig requestConfig = requestConfigBuilder.build();

		Assert.assertEquals(10000, requestConfig.getSocketTimeout());
		Assert.assertEquals(5000, requestConfig.getConnectionRequestTimeout());
