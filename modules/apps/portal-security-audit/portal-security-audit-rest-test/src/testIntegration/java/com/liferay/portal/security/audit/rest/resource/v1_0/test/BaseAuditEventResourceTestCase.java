...
public abstract class BaseAuditEventResourceTestCase {
	...
	public void testEscapeRegexInStringFields() throws Exception {
		...
		Assert.assertEquals(regex, auditEvent.getSessionId());
	}

	@Test
	public void testGetAuditEvent() throws Exception {
		AuditEvent postAuditEvent = testGetAuditEvent_addAuditEvent();

		AuditEvent getAuditEvent = auditEventResource.getAuditEvent(
			postAuditEvent.getId());

		assertEquals(postAuditEvent, getAuditEvent);
		assertValid(getAuditEvent);
	}
	...
	public void testVulcanCRUDItemDelegateGetItem() throws Exception {
		...
		Object item = vulcanCRUDItemDelegate.getItem(postAuditEvent.getId());

		assertEquals(getAuditEvent, AuditEventSerDes.toDTO(item.toString()));
	}
	...
	public void testGetAuditEventByContextNameContextNamePage()
		...
		String irrelevantContextName =
			testGetAuditEventByContextNameContextNamePage_getIrrelevantContextName();
		...
		if (irrelevantContextName != null) {
			AuditEvent irrelevantAuditEvent =
			...
			assertContains(
				irrelevantAuditEvent, (List<AuditEvent>)page.getItems());
			assertValid(
				page,
				testGetAuditEventByContextNameContextNamePage_getExpectedActions(
					irrelevantContextName));
		}
	...
	@Test
	public void testGetAuditEventByContextNameContextNamePageWithSortDateTime()
		throws Exception {

		testGetAuditEventByContextNameContextNamePageWithSort(
			EntityField.Type.DATE_TIME,
	...
	@Test
	public void testGetAuditEventByContextNameContextNamePageWithSortDouble()
		throws Exception {
	...
	@Test
	public void testGetAuditEventByContextNameContextNamePageWithSortInteger()
		throws Exception {
	...
	@Test
	public void testGetAuditEventByContextNameContextNamePageWithSortString()
		throws Exception {
	...
	protected void testGetAuditEventByContextNameContextNamePageWithSort(
	...
	protected AuditEvent
			testGetAuditEventByContextNameContextNamePage_addAuditEvent(
				String contextName, AuditEvent auditEvent)
		throws Exception {
	...
	protected void assertValid(AuditEvent auditEvent) throws Exception {
		...
		for (String additionalAssertFieldName :
			...
			if (Objects.equals("serverPort", additionalAssertFieldName)) {
				if (auditEvent.getServerPort() == null) {
					valid = false;
				}
			...
			if (Objects.equals("sessionId", additionalAssertFieldName)) {
				if (auditEvent.getSessionId() == null) {
					valid = false;
				}
	...
	protected boolean equals(AuditEvent auditEvent1, AuditEvent auditEvent2) {
		...
		for (String additionalAssertFieldName :
			...
			if (Objects.equals("message", additionalAssertFieldName)) {
				if (!Objects.deepEquals(
						auditEvent1.getMessage(), auditEvent2.getMessage())) {
			...
			if (Objects.equals("serverPort", additionalAssertFieldName)) {
				if (!Objects.deepEquals(
						auditEvent1.getServerPort(),
						auditEvent2.getServerPort())) {
			...
			if (Objects.equals("sessionId", additionalAssertFieldName)) {
				if (!Objects.deepEquals(
						auditEvent1.getSessionId(),
						auditEvent2.getSessionId())) {
	...
	protected String getFilterString(
		...
		if (entityFieldName.equals("message")) {
			Object object = auditEvent.getMessage();
		...
		if (entityFieldName.equals("serverPort")) {
			sb.append(String.valueOf(auditEvent.getServerPort()));

			return sb.toString();
		}

		if (entityFieldName.equals("sessionId")) {
			Object object = auditEvent.getSessionId();
		...
		return new AuditEvent() {
			{
				...
				serverName = StringUtil.toLowerCase(
					RandomTestUtil.randomString());
				serverPort = RandomTestUtil.randomInt();
				sessionId = StringUtil.toLowerCase(
					RandomTestUtil.randomString());
			}
		};
	}
