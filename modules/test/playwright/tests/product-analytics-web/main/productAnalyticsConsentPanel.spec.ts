...
test(
	...
	async ({page, usersAndOrganizationsPage}) => {
		...
		await clickAndExpectToBeVisible({
			target: usersAndOrganizationsPage.page.locator('.nav-link', {
				hasText: 'Product Analytics',
			}),
			trigger: page.getByText('Edit User Test Test', {
				exact: true,
			}),
		});

		await expect(
			page.getByText('Edit User Test Test', {exact: true})
		).toBeVisible();
	}
);
