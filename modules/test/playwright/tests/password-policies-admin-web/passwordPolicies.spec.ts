...
test('Edit default password policy with syntax checking and 1 lowercase and check that it shows an error for Minimum Lower Case error',
    {tag: '@LPD-48268'}, async ({
    page,
    ...
    await passwordPoliciesAdminConfigPage.goTo();
    await passwordPoliciesAdminConfigPage.editDefaultPasswordPolicy(passwordPolicy);
    ...
    await page.getByLabel('Email Address').fill(getRandomString()+'@liferay.com');
    ...
    await expect(page.getByText('Close Error: That password must contain at least 1 lowercase characters. User')).toBeVisible();
